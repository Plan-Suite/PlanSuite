                    ...
                    if (this.options.multiplePick) {
                        if (this.days[index].timestamp) {
                            this.daysSelected = this.daysSelected
                                .filter((day) => setToTimestamp(day) !== this.lastSelectedDay);
                        }
            ...
            if (dayOptions.day === 1) {
                if (this.options.weekStart === HelloWeek.daysWeek.SUNDAY) {
                    ...
                    if (day === HelloWeek.daysWeek.SUNDAY) {
                        setStyle(newDay, this.options.rtl ? 'margin-right' : 'margin-left', ((Object.keys(HelloWeek.daysWeek).length - this.options.weekStart) * (100 / Object.keys(HelloWeek.daysWeek).length)) + '%');
                    }
                    else {
